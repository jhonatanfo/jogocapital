...
namespace jogo.capital.Server.Services
{
    public class smsSendpulse : IsmsSendPulse
    {
        ...
        public smsSendpulse(string _userId, string _secret)
        {
        ...
        /// Send viber campaign.
        /// </summary>
        /// <<param name="Campanha">Viber campaign to create</param>
        /// <returns>The viber campaign.</returns>
        public Dictionary<string, object> sendCampanha(Campanha campanha)
        {
            if (campanha.AddressBook == 0 && campanha.Recipients.Length == 0)
                return this.handleError("Empty recipients list");

            if (campanha.Message.Length == 0)
                return this.handleError("Empty message");

            if (campanha.SenderId == 0)
                return this.handleError("Empty sender");

            Dictionary<string, object> result = null;
            try
            {
                result = this.sendJSONRequest("/viber", HttpMethod.Post, campanha);
            }
            catch (IOException) { }
            return this.handleResult(result);
        }
        ...
        /// <param name="id">Identifier.</param>
        public Dictionary<string, object> getCampanhaStat(int id)
        {
            Dictionary<string, object> result = null;
